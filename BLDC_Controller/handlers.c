...
uint32_t cnt_FastEvent = 0, cnt_MedEvent = 0, cnt_SlowEvent = 0,
		cnt_UART_Timeout = 0;
...
extern uint32_t mRPM, mFilteredRPM, free_run_counter;
...
void PIOINT1_IRQHandler(void) {
	...
	free_run_counter++; //indicate the motor is running
	...
	if (regVal & BEMF_pin_mask) {
		...
		corrected_zero_crossing_time = zero_crossing_time
				- config.zc_correction;
		...
		LPC_TMR16B1->MR1 = comm_Time / 2 + corrected_zero_crossing_time;

		if ((zero_crossing_time >= comm_Time / 2)) {
			; // && (comm_Time > START_COMM_TIME/10)) {
...
void Blink(uint8_t count, uint32_t interval) { // blink the LED 3 times
	...
	for (i = 1; i <= count; i++) {
		...
		PHC_LO = pwmPeriod - 200;
		PHB_HI = ON;


		SysTick_cntr = 0;
		while (SysTick_cntr < interval) {
			if (SysTick_cntr & 0x01) {
				PHB_HI = ON;
			} else {
				PHB_HI = OFF;
			}
