...
typedef struct IMU_STRUCT {
	...
	uint16_t AIN1, AIN2, AIN3;		// 3 analogue inputs
	uint16_t AIN_temp;				// Additional temperature sensor
	int16_t temperature; 			// Chip temperature
	uint16_t DIN1;					// 2 bit digital input
	uint16_t UID;					// Board ID
} IMU_REPORT;




#define LSM9DS1_AG_ADDR(sa0) ((sa0) ? 0xD6 : 0xD4)
#define LSM9DS1_M_ADDR(sa1) ((sa1) ? 0x3C : 0x38)
...
typedef enum lsm9ds1_axis lsm9ds1_axis;
