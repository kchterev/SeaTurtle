...
void I2C_IRQHandler(void) 
{
  ...
  switch ( StatValue )
  {
	...
	if ( I2CWriteLength == 1 )
	{
	  LPC_I2C->CONSET = I2CONSET_STO;      /* Set  flag */
...
** Function name:		I2C
**
** Descriptions:		Set the I2C  condition, if the routine
...
**				from start to . All the intermitten
...
**                            End Of File
******************************************************************************/
uint32_t i2c_write(uint8_t Address, uint8_t Subaddress, uint8_t* data, uint8_t len){

	  I2CWriteLength = 2 + len;
	  I2CReadLength = 0;
	  I2CMasterBuffer[0] = Address;
	  I2CMasterBuffer[1] = Subaddress;		/* address */
	  I2CMasterBuffer[2] = data[0];		/* Data0 */
	  I2CMasterBuffer[3] = data[1];		/* Data1 */
	  I2CMasterBuffer[4] = data[2];		/* Data2 */
	  I2CMasterBuffer[5] = data[3];		/* Data3 */
	  I2CMasterBuffer[6] = data[4];		/* Data4 */
	  I2CMasterBuffer[7] = data[5];		/* Data5 */
	  I2CMasterBuffer[8] = data[6];		/* Data6 */
	  I2CMasterBuffer[9] = data[7];		/* Data5 */

	  I2CEngine();
	  return TRUE;
}
uint32_t i2c_read (uint8_t Address, uint8_t Rx_SubAddress, uint8_t len){

	  I2CWriteLength = 0;
	  I2CReadLength = len;
	  I2CMasterBuffer[0] = Address;
	  I2CMasterBuffer[1] = Rx_SubAddress;		/* address */

	  I2CEngine();
	  return TRUE;
}

uint32_t i2c_write_read (uint8_t Tx_Address, uint8_t Tx_SubAddress, uint8_t Tx_len,
		uint8_t * data, uint8_t Rx_len){

		uint32_t res;

		I2CWriteLength = Tx_len;
		I2CReadLength = Rx_len;
		I2CMasterBuffer[0] = Tx_Address;
		I2CMasterBuffer[1] = Tx_SubAddress;
		I2CMasterBuffer[2] = Tx_Address | RD_BIT;
