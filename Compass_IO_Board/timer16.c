...
extern int32_t pwmSP_SERVO[6]; //6 pwm outs

int32_t PhC_Min, PhC_Max;

uint16_t PingerCounter;
...
void TIMER16_0_IRQHandler(void) {
	...
	if ( LPC_TMR16B0->IR & 1 << 0) {
		LPC_TMR16B0->IR |= 1 << 0; // clear interrupt flag
	}
	if (LPC_TMR16B0->IR & 1 << 1) {
		LPC_TMR16B0->IR |= 1 << 1; // clear interrupt flag

		PingerCounter++;

		if(PingerCounter >= pwmSP_SERVO[1]+1){
			LPC_TMR16B0->TCR |= 0x02; //reset the counter
		}
...
void TIMER16_1_IRQHandler(void) {
	...
	if ( LPC_TMR16B1->IR & (1 << 1)) {
		...
		if (stl_tmp < config.stall_phase_voltage){
...
void init_timer16PWM(uint8_t timer_num, uint32_t period, uint8_t match_enable,
	...
	if (timer_num == 1) {
		...
		//LPC_TMR16B0->MCR |= 1 << 0;		// interrupt on MR0
		LPC_TMR16B0->MCR |= 1 << 3;		// interrupt on MR1
