...
void LSM9DS1()
{
	LSM9DS1_init(IMU_MODE_I2C, LSM9DS1_AG_ADDR(1), LSM9DS1_M_ADDR(1));
}

//LSM9DS1(interface_mode interface, uint8_t xgAddr, uint8_t mAddr)
//{
//	LSM9DS1_init(interface, xgAddr, mAddr);
//}
...
void initMag()
{
    ...
    tempRegValue = 1; //enable MAG interrupt output
    mWriteByte(INT_SRC_M, tempRegValue);
}
...
uint8_t magAvailable(lsm9ds1_axis axis)
{
    ...
    data = mReadByte(STATUS_REG_M);
    
    return ((data & (1<<axis)) >> axis);
}
...
void I2CwriteByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
    ...
    */
    //uint8_t temp_data[2] = {subAddress, data};
...
uint8_t I2CreadByte(uint8_t address, uint8_t subAddress)
{
    ...
    */
	uint8_t data[1];
    //uint8_t temp[1] = {subAddress};
    
    //i2c_write(address, temp, 2);
    //i2c_write(address & 0xFE);
    //temp[1] = 0x00;
    //i2c_write(address, temp, 1);
    //i2c_write( address | 0x01);
    i2c_write_read(address, subAddress, 0, data, 1);
...
{  
    ...
    */
    ...
    //uint8_t temp[1] = {subAddress};
    //i2c_write(address, temp, 1);
    //i2c_read(address, temp_dest, count);
    
	i2c_write_read(address,subAddress,0,dest, count);


    //i2c doesn't take uint8_ts, but rather uint8_ts so do this nasty af conversion
    //for (i=0; i < count; i++) {
    //    dest[i] = temp_dest[i];
    //}
    return count;
