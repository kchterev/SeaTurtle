...
/****************************************************/
//init params
uint32_t PWM_Servo;			//Servo frequency in Hz
uint32_t PWM_Lights;		//PWM frequency for the lights in Hz
uint32_t PWM_Pinger;			//Pinger frequency in Hz

//Variable params
uint32_t pwmPeriodServo;			//PWM total pulse length register value
uint32_t pwmPeriodLights;
uint32_t pwmPeriodPinger;
...
uint32_t RPM_Const, Min_Comm_Time;

//Timers prescaler

uint32_t TMRPrescalerServo;
uint32_t TMRPrescalerLights;
uint32_t TMRPrescalerPinger;
...
// PWM
uint32_t pwmZeroSP;
int32_t pwmSP_SERVO[6]; //6 pwm outs
...
void vBLDC_Init(void) {
	...
	// Servo pwm output = 50Hz
	PWM_Servo = 50;
	PWM_Lights = 1000;
	PWM_Pinger = 40000;
	TMRPrescalerServo = 32;
	TMRPrescalerLights = 0;
	TMRPrescalerPinger = 0;
	...
	/****************************************************/
	pwmPeriodPinger = SystemCoreClock / (TMRPrescalerPinger + 1) / PWM_Pinger; //PWM total pulse length register value = 0% PWM (4800/12=4000)
	pwmPeriodServo = SystemCoreClock / (TMRPrescalerServo + 1) / PWM_Servo; //PWM total pulse length register value = 0% PWM (4800/12=4000)
	pwmPeriodLights = SystemCoreClock / (TMRPrescalerLights + 1) / PWM_Lights; //PWM total pulse length register value = 0% PWM (4800/12=4000)




	pwmZeroSP= pwmPeriodServo  * 92.5/100; // servo pwm zero SP = 7.5% pwm

	pwmMaxPeriod = pwmPeriodServo * 0.96; //PWM max allowed pulse length register value = 10%PWM (3600)
	...
	init_timer32PWM(TIMER0, pwmPeriodServo, (MATCH0 | MATCH2 | MATCH3));
	/* Setup Phase B high and low side PWM signals */
	init_timer16PWM(TIMER0, pwmPeriodPinger, MATCH0, CAP_DISABLE);
	/* Setup Phase C high and low side PWM signals */
	init_timer32PWM(TIMER1, pwmPeriodLights, (MATCH0 | MATCH1));

	pwmLoadVal = pwmMaxPeriod; // make the PWM PID output = minimum

	//pre-scale all PWM timers

	LPC_TMR32B0->PR = TMRPrescalerServo;
	LPC_TMR16B0->PR = TMRPrescalerPinger;
	LPC_TMR32B1->PR = TMRPrescalerLights;
	...
	//set servos to middle point
	PHA_HI = pwmZeroSP;
	PHA_LO = pwmZeroSP;
	PHB_HI = pwmPeriodPinger/2;
	PHB_LO = pwmPeriodPinger/2-300;
	PHC_HI = 0xFFFF;
	PHC_LO = 0xFFFF;
...
void vBLDC_LoadMatch(void) {
	...
	PHA_HI = pwmSP_SERVO[0];
	PHA_LO = pwmSP_SERVO[3];
	//PHB_HI = pwmPeriodPinger /2 ; //pwmSP_SERVO[2];
	//PHB_LO = pwmPeriodPinger /2 ; //pwmSP_SERVO[1];
	PHC_HI = pwmSP_SERVO[5];
	PHC_LO = pwmSP_SERVO[4];
...
void set_config() {
	...
	B_Voltage = 24;			//Board voltage = 24V

	M_Kv = 280;				//Motor Kv = 150 (Turnigy 280kV)
	M_MaxV = 24;			//Motor maximum voltage = 15V (Turnigy 280kV)
	M_PolePairs = 7;		//Motor pole pairs = 7 (Turnigy 280kV)

	M_MaxRPM = B_Voltage * M_Kv; 	//Motor max RPM

	oFastEvent = 30;			//Fast Event time cycle = 50 ms
	oMedEvent =  150;			//Med Event time cycle  = 50 ms
	oSlowEvent = 500;			//Slow Event time cycle = 1s
	oUARTtimeout = 25;			//UART Timeout = 25 ms
...
{
uint16_t i, checksum;
	config.board_id = 7;			//Board ID - 0 and 0xFF - reserved, 1 onwards - OK.
									// MUST BE UNIQUE FOR EACH BOARD !!!!!!!!

	config.allign_duty = 8; 		//This duty cycle defines the momentum the motor will apply when started
	...
	config.ramp_duty = 12;			//The RAMP_DUTY defines the momentum of the ramping stage.
									//Must be close to ALIGN_DUTY
									//In %	!!!!!!!!!!!! IMPORTANT TO START !!!!!!!!!!!!!!!!!

	config.delta_sp = 50;			//defines the set point increment for soft start
									//At the end of the ramping the duty is different than the start
									//smaller if no load, larger if heavier load is applied

	config.stop_delay = 150;		//Defines the time in STOP state. in mS;
	config.align_delay = 150;		//Defines the time in ALLIGN state. in mS;
	config.start_comm_time = 5000;	//Initial commutatation time. in uS;

	config.blanking_time = 100;		//Zero Crossing check Blanking Time
	...
	config.zc_stall_count = 150;	//Counts the low delta Phase Voltage events.
	...
	config.min_rpm = 99;			//Motor min RPM = 180 (Turnigy 280kV)

	config.p1 = 1100;				//Magnitometer X_AXIS offset
	config.p2 = 475;				//Magnitometer Y_AXIS offset
	config.p3 = 8;					//Magnitometer scale(sensitivity) - 4,8,12 or 16
	config.p4 = 0;
	config.p5 = 0;					//Some spare configuration parameters.
	config.p6 = 0;
	config.p7 = 0;
	config.p8 = 0;
	config.checksum = 0;
